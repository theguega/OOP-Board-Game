...
void LastPartieBuilder::setJoueurs() const {
    ...
    //Connexion et ouverte de la base de donnees
    ...
    //On instancie les 2 anciens joueurs avec leurs privileges
...
void LastPartieBuilder::setCartesJoueurs() const {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (rc2 != SQLITE_OK) {
            std::cerr << "Erreur de preparation de la requête joueur 3 : " << sqlite3_errmsg(db) << std::endl;
            sqlite3_close(db);
            return;
        }
...
//    // creation et affectation de nouveaux joueurs 
